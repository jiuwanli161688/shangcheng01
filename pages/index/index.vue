<template>
  <view class="page-index">
    ...
    <scroll-view
      class="page-content"
      scroll-y
      :style="{ height: scrollViewHeight, marginTop: getNavBarHeight() + 'px' }"
      ...
      <view class="notice-section" v-if="noticeList.length > 0">
        ...
        >
          <swiper-item v-for="(notice, index) in noticeList" :key="index">
            <text class="notice-text" @click="handleNoticeClick(notice)">{{
              notice.title
            }}</text>
          </swiper-item>
        </swiper>
    ...
    getNavBarHeight() {
      ...
      try {
        if (uni.getMenuButtonBoundingClientRect) {
          ...
          const systemInfo = uni.getSystemInfoSync();
          // 计算导航栏高度
          if (systemInfo.statusBarHeight && menuButtonInfo.top) {
            navBarHeight =
      ...
      return navBarHeight + this.statusBarHeight;
    },
    // 加载首页数据
    async loadHomeData() {
      try {
